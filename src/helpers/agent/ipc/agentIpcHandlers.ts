...
export function registerAgentIpcHandlers(ipcMain: any, shell?: any): void {
  ...
  // TokenService lives inside McpClientService (accessible via reflection/instance)
  ...
  /** Build OAuth authorize URL and optionally launch browser. */
  ipcMain.handle('agent:login', async () => {
    try {
      const { url, state } = await tokenService.buildAuthorizeUrl();
